...
start: function(){
	...
	if (localStorage["contextMenuButton"] == "on"){
		var contextMenuButton = chrome.contextMenus.create({"title": "Рассказать друзьям", "contexts": ["selection"], "onclick": VKBBase.selectionPost});
	}
...
get_count: function(url, tab){
	var count_url = "http://vk.com/share.php?act=count&index=0&url="+url;
	$.ajax({
	  url: count_url,
	  type: "GET",
	  dataType: "html",
...
popup: function(tab){
	...
	var widgetUrl = 'http://vk.com/share.php';
		widgetUrl += '?Share_VK';
